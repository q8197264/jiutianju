...
if($_GET['act'] == 'save' && $_GET['formhash'] == FORMHASH){
    ...
    $attrnameArr = $attrArr = $attrdateArr = $attrdatetmpArr = $tagnameArr = $photoArr = array();
    foreach($_GET as $key => $value){
        ...
        if(strpos($key, "attrdatetmp_") !== false){
            $attr_id = intval(ltrim($key, 'attrdatetmp_'));
            $attrdatetmpArr[$attr_id] = addslashes($value);
        }
    ...
    if(C::t('#tom_tongcheng#tom_tongcheng')->update($tongcheng_id,$updateData)){
        ...
        if(is_array($attrdateArr) && !empty($attrdateArr)){
            foreach ($attrdateArr as $key => $value){
                if(empty($value)){
                    if(!empty($attrdatetmpArr[$key])){
                        $value = $attrdatetmpArr[$key];
                    }
                }
                $insertData = array();
                $insertData['model_id']     = $model_id;
    ...
    $tongchengPhotoListTmp = C::t('#tom_tongcheng#tom_tongcheng_photo')->fetch_all_list(" AND tongcheng_id={$tongchengInfo['id']} "," ORDER BY id DESC ",0,50);
