...
if(isset($_SESSION['tom_kanjia_openid']) && !empty($_SESSION['tom_kanjia_openid'])){
    $openid = $_SESSION['tom_kanjia_openid'];
    $nickname = $_SESSION['tom_kanjia_nickname'];
    $headimgurl = $_SESSION['tom_kanjia_headimgurl'];
    
    $common_access_token = $weixinClass->get_access_token();
    if(!empty($common_access_token)){
        $check_return = check_subscribe($common_access_token,$openid);
        if($check_return){
            $subscribe = 1;
        }
    }
    ...
    if(isset($_GET['code']) && !empty($_GET['code'])){
        ...
        $common_access_token = $weixinClass->get_access_token();
        if(!empty($openid) && !empty($access_token)  && !empty($common_access_token)){
            ...
            if($oauth2_snsapi_userinfo && isset($oauth2_snsapi_userinfo['nickname'])){
                ...
                $_SESSION['tom_kanjia_openid'] = $openid;
                $_SESSION['tom_kanjia_nickname'] = $nickname;
                $_SESSION['tom_kanjia_headimgurl'] = $headimgurl;
