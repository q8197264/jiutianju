...
class table_tom_love_renzheng extends discuz_table{
	public function __construct() {
        parent::__construct();
		$this->_table = 'tom_love_renzheng';
		$this->_pk    = 'id';
	}
    ...
    public function fetch_by_uid($uid,$field='*') {
		return DB::fetch_first("SELECT $field FROM %t WHERE user_id=%d ", array($this->_table, $uid));
	}
    ...
    public function update_renzheng_status($status,$uid) {
		return DB::query("UPDATE %t SET renzheng_status=%d WHERE user_id=%d", array($this->_table, $status,$uid));
	}
