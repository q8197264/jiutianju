...
if(submitcheck('editsubmit')) {
	...
	$delids = array();
	foreach(C::t('#tom_love#tom_love_district')->fetch_all_by_upid($theid) as $value) {
		if(!isset($_POST['district'][$value['id']])) {
			$delids[] = $value['id'];
		} elseif($_POST['district'][$value['id']] != $value['name'] || $_POST['displayorder'][$value['id']] != $value['displayorder']) {
			C::t('#tom_love#tom_love_district')->update($value['id'], array('name'=>$_POST['district'][$value['id']], 'displayorder'=>$_POST['displayorder'][$value['id']]));
		}
	}
	if($delids) {
		$ids = $delids;
		for($i=$level; $i<4; $i++) {
			$ids = array();
			foreach(C::t('#tom_love#tom_love_district')->fetch_all_by_upid($delids) as $value) {
		...
		C::t('#tom_love#tom_love_district')->delete($delids);
	}
	if(!empty($_POST['districtnew'])) {
		...
		foreach($_POST['districtnew'] as $key => $value) {
			...
			$value = trim($value);
			if(!empty($value)) {
				C::t('#tom_love#tom_love_district')->insert(array('name' => $value, 'level' => $level, 'upid' => $theid, 'displayorder' => $displayorder));
			}
		}
	}
    ...
    if($jyConfig['district_importall'] == 1){
        $sql='TRUNCATE TABLE '.DB::table('tom_love_district');
        DB::query($sql);
    }
    
    $sql='INSERT INTO '.DB::table('tom_love_district').'(id,name,level,upid,displayorder) SELECT id,name,level,upid,displayorder FROM '.DB::table('common_district');
    DB::query($sql);
    
    cpmsg($pluginScriptLang['district_importall_success'], $modListUrl, 'succeed');
}else{
	showtips('district_tips');
    echo '&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.$modBaseUrl.'&act=importall&formhash='.FORMHASH.'" class="addtr">'.$pluginScriptLang['district_importall'].'</a><br/>';
	...
	$thevalues = array();
	foreach(C::t('#tom_love#tom_love_district')->fetch_all_by_upid($upids) as $value) {
