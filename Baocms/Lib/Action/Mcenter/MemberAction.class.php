...
class MemberAction extends CommonAction{
    public function pay(){
        ...
        } elseif ($detail['type'] == 'pao') {
            $this->pao_success('恭喜您支付成功啦！', $detail);
    ...
    protected function pao_success($message, $detail) {
        $obj = D('PaoRecords');
        $paoorder = $obj->where("record_id='".$detail['order_id']."'")->find();
        $this->assign('paoorder', $paoorder);
        $this->assign('message', $message);
        $this->assign('detail', $detail);
        $this->assign('paytype', D('Payment')->getPayments());
        $this->display('pao');
    ...
    protected function ele_success($message, $detail){
        ...
        //jack add 2017/8/25
        //D('Sms')->eleTZshopPSY($order_id);//jack add 2017/8/25
        //end
    ...
    public function dosq(){
        ...
        if(!$username){
            $this->fengmiMsg('帐号没有填写！');
        }
        if(!$password || strlen($password)<6){
            $this->fengmiMsg('密码错误或小于6位！');
        }
        if(!$rpw || strlen($rpw)<6){
            $this->fengmiMsg('确认密码错误或小于6位！');
        }
        ...
        if(!$name){
            $this->fengmiMsg('姓名没有填写！');
        }
        
        if(!$mobile || strlen($mobile) != 11){
            $this->fengmiMsg('手机号填写错误！');
        }
        ...
        if($fu){
            $this->fengmiMsg('重复的帐号！');
        }
        
        $fm = $dv -> where('mobile ='.$mobile) -> find();
        if($fm){
            $this->fengmiMsg('重复的手机号！');
        }
        ...
        $r = $dv -> add($result);
        if($r){
            $this->fengmiMsg('提交申请成功，等待审核', U('member/index'));
        }else{
            $this->fengmiMsg('添加失败！');
        }
    }
    ...
    {        
        ...
        if(!empty($delivery_info['id'])){
            if($delivery_info['d_status']==1){
                $this->error('您的账户已经绑定配送员了！');
            }
            else{
                $this->error('您的账户正式审核中，请耐心等待！');
            }
            exit;
        }
        $users = M("users");
