...
class CouponAction extends CommonAction{
    ...
    public function give(){
		$download_id = $this->_get('download_id');
		   ...
		   if (!$detail = D('Coupondownload')->find($download_id)) {
            $this->fengmiMsg('没有该优惠券');
			}
			if ($detail['user_id'] != $this->uid) {
				$this->fengmiMsg("优惠券不存在！");
			}
			if ( $detail['is_used'] != 0) {
				$this->error('该优惠券属于不可消费的状态');
			}
            ...
            if (D('Coupondownload')->register_account_give_coupon($download_id,$mobile)){
                    $this->fengmiMsg('恭喜您赠送成功快去通知你的好友吧！', U('coupon/index'));
                }else{
                    $this->fengmiMsg('注册账户操作失败');
                }}else{
					$obj = D('Coupondownload');
			if($obj->save(array('download_id' => $download_id, 'user_id' => $user['user_id']))){
			
			$this->fengmiMsg('恭喜您赠送成功', U('/user/coupon/index/'));}
