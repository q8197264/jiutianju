...
class EleAction extends CommonAction
{
    ...
    public function order()
    {
        ...
        foreach ($num as $key => $val) {
            ...
            $total['canhe_price'] += $product['canhe_price']*$val;
            $total['num'] += $val;
        }
        if (count($shops) > 1) {
		...
		$settlement_price = (int) ($total['money'] - ($total['money']* $shop['rate'] / 1000)+$total['canhe_price']);//先算出结算价，不按照后台为准
        $total['money'] += $shop['logistics']+$total['canhe_price'];
    ...
    public function pay2()
    {
        ...
        if ($code == 'wait') {
            ...
            D('Sms')->eleTZshop($order_id);//jack add 2017/8/25
