...
class MartAction extends CommonAction{
    ...
    public function all(){
        ...
        if(!empty($list)){
            ...
            foreach($list as $key => $val){
                $chck = $delivery_order->field("order_id,is_confirm")->where("type=0 and type_order_id='".$val['order_id']."' and delivery_id<1")->find();
                if(!empty($chck)){
    ...
    public function confirmxp(){
        ...
        if($id>0){
            $delivery_order = D("DeliveryOrder");
            $delivery_order->where("type=0 and type_order_id='".$id."' and delivery_id<1")->save(array('is_confirm'=>1));
