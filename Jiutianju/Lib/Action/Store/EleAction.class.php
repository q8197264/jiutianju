...
class EleAction extends CommonAction{
    private $create_fields = array('product_name', 'desc', 'cate_id', 'photo', 'price', 'is_new', 'is_hot', 'is_tuijian', 'create_time', 'create_ip','canhe_price');
    private $edit_fields = array('product_name', 'desc', 'cate_id', 'photo', 'price', 'is_new', 'is_hot', 'is_tuijian','canhe_price');
    ...
    public function eleorder_lada_data(){
        ...
        if(!empty($list)){
            ...
            foreach($list as $key => $val){
                $chck = $delivery_order->field("order_id,is_confirm")->where("type=1 and type_order_id='".$val['order_id']."' and delivery_id<1")->find();
                if(!empty($chck)){
                    if($chck['is_confirm']==1){
                        $list[$key]['is_confirm'] = 1;
                    }
                    else{
                        $list[$key]['is_confirm'] = 0;
                    }
                }
                else{
                    $list[$key]['is_confirm'] = 1;
    ...
    public function confirmxp(){
        $id = I("post.id");
        if($id>0){
            $delivery_order = D("DeliveryOrder");
            $delivery_order->where("type=1 and type_order_id='".$id."' and delivery_id<1")->save(array('is_confirm'=>1));
            D('Sms')->eleTZshopPSY($id);
            echo '1';
            exit;
        }
        else{
    ...
    private function createCheck()
    {
        ...
        $data['canhe_price'] = (int) ($data['canhe_price'] * 100);
