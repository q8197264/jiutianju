...
class CouponAction extends CommonAction
{
    ...
    {
		$todayEnd= date('Y-m-d ', time());
		$counts1= D('Coupon')->where(array('shop_id' => $this->shop_id, 'expire_date' => array('EGT',$todayEnd)))->count();
    if ($counts1 > 0) {
    ...
    private function createCheck(){
		...
		$au=$this->_CONFIG['coupon']['coupon_audit'];
        $data['audit'] = $au;
        return $data;
    }
    ...
    private function editCheck()
    {
		...
		$data['rank_id'] = htmlspecialchars($data['rank_id']);
        $data['limit_num'] = (int) $data['limit_num'];
		$au=$this->_CONFIG['coupon']['coupon1_audit'];
        $data['audit'] = $au;
        return $data;
    }
}