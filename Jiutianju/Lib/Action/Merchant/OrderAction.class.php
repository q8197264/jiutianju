...
class OrderAction extends CommonAction {
    ...
    public function refund($order_id = 0){
	 ...
	 //只支持单个退款
    public function fanli($order_id = 0){
        $order_id = (int) $order_id;
		$order = D('Order');
        $detail = $order->find($order_id);
		 $count = D('Userintegralrestore')->where(array('order_id' => 28))->count();
		 if ($count=0){
        ...
        if ($detail['is_daofu'] == 0) {
			...
			D('Userintegrallibrary')->Add(array('order_id'=>$detail[order_id],'user_id'=>$detail['user_id'],'integral_library'=>1000,'integral_library_surplus'=>1000,'integral_library_day'=>100,'integral_library_total'=>10,'intro'=>'商城分时全额返还，商户申请','create_time'=>NOW_TIME,'create_ip'=>get_client_ip()));
			D('Users')->addMoney($detail['user_id'],-100,'申请返现积分扣除服务费');
