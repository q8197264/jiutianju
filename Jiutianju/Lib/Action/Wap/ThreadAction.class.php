...
class ThreadAction extends CommonAction {
    ...
    public function postdetail($post_id){
        if(!$post_id = (int)$post_id){
			...
			$this->assign('reply_num',$reply_num= D('Threadpostcomments')->where(array('post_id'=>$post_id,'closed'=>0))->count());
    ...
    public function postload(){
            ...
            $lists = D('Threadpostcomments')->where(array('post_id'=>$post_id,'type'=>array('IN',array(0,2))))->order(array('create_time'=>'desc'))->select();
            $count = count($lists);  
        ...
        if(!$thread_id = (int)$thread_id){
            ...
            if($this->isPost()){
                ...
                if($post_id = D('Threadpost')->add($data)){
					...
					$this->fengmiMsg('恭喜您发帖成功',U('thread/postdetail',array('post_id'=>$post_id)));
