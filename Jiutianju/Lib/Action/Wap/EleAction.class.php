...
class EleAction extends CommonAction{
    ...
    public function index(){
       ...
       $this->assign('product', D('Ele_product')->where(array( 'closed' => 0,'is_tuijian' => 1))->select());
    ...
    public function shop(){
        ...
        $map2 = array('closed' => 0, 'audit' => 1, 'shop_id' => $shop_id,'is_tuijian' => 1);
      if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
            $map['product_name'] = array('LIKE', '%' . $keyword . '%');
            $this->assign('keyword', $keyword);
        }
        $list = $Eleproduct->where($map)->order(array('sold_num' => 'desc', 'price' => 'asc'))->select();
        $list2 = $Eleproduct->where($map)->order(array('sold_num' => 'desc', 'price' => 'asc'))->select();
        foreach ($list as $k => $val) {
       ...
       $this->assign('list2', $list2);
        ...
        $this->assign('cate', D('Elecate')->where(array('shop_id' => $shop_id, 'closed' => 0,'num' => array('EGT', 1)))->select());
        $this->assign('tuijian', D('Ele_product')->where(array('shop_id' => $shop_id, 'closed' => 0,'is_tuijian' => 1))->select());
    ...
    public function order(){
		...
		$total['full_reduce_price'] = D('Eleorder')->get_full_reduce_price($total['money'],$shop_id);//获取满减
	    ...
	    if($shop['is_ele_pei']==0){
				...
				$settlement_price1 = $settlement_price +  $logistics ;
			
		}else{
			
			$settlement_price1 = $settlement_price ;
		}
		
		$total['need_pay'] = $total['need_pay'] - $total['new_money'] - $total['logistics_full_money'] - $total['reduce_coupun_money']- $total['full_reduce_price']+ $shop['cj_money'];
        ...
        if ($order_id = D('Eleorder')->add(array(
            ...
            'logistics' => $logistics, 
			'settlement_price' => $settlement_price1, 
			'status' => 0,
			'cj_money' => $shop['cj_money'],
    ...
    public function pay2() {
        ...
        D('Eleorder')->save(array('order_id' => $order_id,'addr_id' => $addr_id));
