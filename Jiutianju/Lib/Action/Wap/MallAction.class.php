   ...
   public function get_spec($goods_id){
 	...
 	foreach($goods_ids as $v){
       ...
       if (empty($goods)) {
		...
		$num[$val['goods_id']] = $num[$val['goods_spec']];
            ...
            $coupon_price = D('Coupon')->Obtain_Coupon_Price($order_id,$download_id);//获取优惠券减的钱
		...
		if(!empty($download_id)){
			...
			if(!empty($coupon_price)){
				 D('Order') ->where("order_id={$order_id}")->save(array('coupon_price' =>$coupon_price,'download_id' =>$download_id));
			}
		}
        ...
        if (count($order_ids) > 1) {
            $need_pay1 = D('Order')->useIntegral($this->uid, $order_ids);
            $need_pay = $need_pay1 - $conponyouhiu;
            $logs = array(
				'type' => 'goods', 
				'user_id' => $this->uid, 
				'order_id' => 0, 
    ...
    public function pay(){
		...
		if($Paymentlogs['need_pay']){
			...
			$need_pay = $order['total_price'] + $order['express_price'] -$coupon_price - $use_integral - $order['mobile_fan'];
		}
