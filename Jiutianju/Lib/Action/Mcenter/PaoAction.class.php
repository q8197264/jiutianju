...
class PaoAction extends CommonAction {
    ...
    public function addpao() {
        if ($this->isPost()) {
            ...
            if(empty($check) || $check['category_price']>$data['record_price']){
                $this->fengmiMsg('出价不可低于该分类最低'.$check['category_price'].'报价！');
            }
            ...
            if($id>0){
                ...
                $this->fengmiMsg('数据提交成功，等待去付款', $backurl);
            }
            $this->fengmiMsg('操作失败！');
        } else {
            $pao_category = D("PaoCategory");
    ...
    public function editpao() {
        ...
        if ($id>0) {
            ...
            if ($this->isPost()) {
                ...
                $check = $pao_category->field("category_price")->where("category_id='".$data['category_id']."'")->find();
                if(empty($check) || $check['category_price']>$data['record_price']){
                    $this->fengmiMsg('出价不可低于该分类最低'.$check['category_price'].'元报价！');
                }
                $vo = $obj->where("record_id='".$id."' and fabu_uid='".$this->uid."'")->save($data);
                if (false !== $vo) {
                    $this->fengmiMsg('操作成功', U('pao/index'));
                }
                $this->fengmiMsg('操作失败');
            } else {
                $pao_category = D("PaoCategory");
    ...
    public function deletepao() {
        ...
        if($id>0){
            ...
            $this->fengmiMsg('删除成功！', U('pao/index'));
        }
        else{
            $this->fengmiMsg('请选择要删除的信息');
        }
    }
    ...
    public function confirmpao() {
        ...
        if($id>0){
            ...
            if(false===$vo){
                ...
                $this->fengmiMsg('确认成功！', U('pao/index'));
            }
        }
        else{
            $this->fengmiMsg('请选择要删除的信息');
    ...
    public function order() {
        ...
        if(!empty($order) && $detail['record_id']==$order){
            $this->fengmiMsg('订单提交成功！', U('pao/pay', array('rid' =>$detail['record_id'])));
        }
        else{            
            $this->fengmiMsg('订单提交失败！');
        }
    }
    ...
    public function pay() {
        ...
        $paymentlist = D('Payment')->getPayments(true);
