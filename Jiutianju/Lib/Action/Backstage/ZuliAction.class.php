...
class ZuliAction extends CommonAction {
    ...
    private $create_fields = array('id', 'shop_id','title','start_time', 'end_time','pic_url', 'everyone_prize', 'add_msg', 'content', 'share_logo', 'share_title', 'share_desc', 'must_gz', 'muse_gz_help', 'guanzu_desc', 'guanzu_url', 'add_time', 'paixu', 'status', 'audit');
    private $edit_fields = array('id', 'shop_id','title','start_time', 'end_time','pic_url', 'everyone_prize', 'add_msg', 'content', 'share_logo', 'share_title', 'share_desc', 'must_gz', 'muse_gz_help', 'guanzu_desc', 'guanzu_url', 'add_time', 'paixu', 'status', 'audit');
 
    public function index() {
        $Tomrenqi = D('Tom_weixin_zl');
   ...
   public function audit($id=0 ) {
        ...
        if (is_numeric($id) && ($id = (int) $id)) {
            if($obj->where(array('id' => $id))->save(array('audit' => 1))){
				$this->baoSuccess('审核成功！', U('zuli/index'));
			}else{
				$this->baoError('审核失败');
			}
        } else {
            ...
            $this->baoError('请选择要审核的助力活动');
        }
    }
 public function fh($id=0 ) {
        $obj = D('Tom_weixin_zl_user');
        if (is_numeric($id) && ($id = (int) $id)) {
            if($obj->where(array('id' => $id))->save(array('status' => 0))){
				$this->baoSuccess('封号成功！', U('renqi/index'));
			}else{
				$this->baoError('封号失败');
			}
        } else {
    ...
    public function jf($id=0 ) {
        ...
        if (is_numeric($id) && ($id = (int) $id)) {
            if($obj->where(array('id' => $id))->save(array('status' => 1))){
				$this->baoSuccess('解封成功！', U('renqi/index'));
			}else{
				$this->baoError('解封失败');
			}
        } else {
