    ...
    public function index(){
        ...
        if ($rank_id = (int) $this->_param('rank_id')) {
      	...
      	if (isset($_GET['is_aux']) || isset($_POST['is_aux'])) {
            $is_open = (int) $this->_param('is_aux');
            if ($is_open != 999) {
                $map['is_aux'] = $is_open;
            }
            $this->assign('is_aux', $is_open);
        } else {
            $this->assign('is_aux', 999);
        }
      if (isset($_GET['is_lock']) || isset($_POST['is_lock'])) {
            $is_open = (int) $this->_param('is_lock');
            if ($is_open != 999) {
                $map['is_lock'] = $is_open;
            }
            $this->assign('is_lock', $is_open);
        } else {
            $this->assign('is_lock', 999);
        }
      if (isset($_GET['is_backers']) || isset($_POST['is_backers'])) {
            $is_open = (int) $this->_param('is_backers');
            if ($is_open != 999) {
                $map['is_backers'] = $is_open;
            }
            $this->assign('is_backers', $is_open);
        } else {
