...
class MenuAction extends CommonAction
{
	...
	public function is_show($menu_id){
        $menu_id = (int) $menu_id ;
            $obj = D('Menu');
			$menu=$obj->where(array('menu_id' => $menu_id))->find();
		    $is_show=$menu['is_show'];
			
			if($menu_id==1){
			$this->baoError('系统功能，不能关闭！');
            }
			if($menu_id==11){
			$this->baoError('系统功能，不能关闭！');
            }
			if ($is_show==0){
            $obj->save(array('menu_id' => $menu_id, 'is_show' => 1));
			delFileByDir(APP_PATH . 'Runtime/');
			 $this->baoSuccess('开启成功！', U('menu/index'));
			 
			} else{
            $obj->save(array('menu_id' => $menu_id, 'is_show' => 0 ));
			delFileByDir(APP_PATH . 'Runtime/');
			 $this->baoSuccess('隐藏成功！', U('menu/index'));
			}
	}      
