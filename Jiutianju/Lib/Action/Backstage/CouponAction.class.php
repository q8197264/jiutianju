...
{
    private $create_fields = array('shop_id','shop_nane','title', 'photo', 'full_price', 'reduce_price', 'expire_date', 'num', 'limit_num', 'intro','rank_id', 'full_price2', 'reduce_price2', 'full_price3', 'reduce_price3');
    private $edit_fields = array('shop_id','shop_nane','title', 'photo', 'full_price', 'reduce_price', 'expire_date', 'num', 'limit_num', 'intro','rank_id', 'full_price2', 'reduce_price2', 'full_price3', 'reduce_price3');
    ...
    {
		$counts1= D('Coupon')->where(array('shop_id' => $this->shop_id, 'closed' => 0))->count();
    if ($counts1 > 0) {
            $this->error('您的店铺当前还存在未过期的优惠券项目，请等优惠券失败后，再来申请添加！', U('coupon/index'));
        }
        if ($this->isPost()) {
			...
			$this->assign('rank', D('Userrank')->fetchAll());
            $this->display();
        }
    }
    private function createCheck()
    {
       ...
       $data['full_price2'] = (int) ($data['full_price2'] * 100);
      if ($data['full_price2'] <= $data['full_price']) {
            $this->baoError('条件1的满多少钱大于条件2所设置的');
        }
      $data['reduce_price2'] = (int) ($data['reduce_price2'] * 100);
       if ($data['reduce_price2'] >= $data['full_price2']) {
            $this->baoError('条件2的减多少钱不能大于减多少钱');
        }
       $data['full_price3'] = (int) ($data['full_price3'] * 100);
      if ($data['full_price3'] <= $data['full_price2']) {
            $this->baoError('条件2的满多少钱大于条件3所设置的');
        }
      $data['reduce_price3'] = (int) ($data['reduce_price3'] * 100);
       if ($data['reduce_price3'] >= $data['full_price3']) {
            $this->baoError('条件3的减多少钱不能大于减多少钱');
        }
        $data['num'] = (int) $data['num'];
		$data['rank_id'] = htmlspecialchars($data['rank_id']);
