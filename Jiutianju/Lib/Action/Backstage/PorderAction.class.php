	...
	public function tuikuan($id = 0) {
		if (is_numeric($id) && ($id = (int)$id)) {
			...
			if ($detail['order_status'] == 7 && !empty($detail['order_no']) && $detail['pay_name'] == 'weixin') {
				...
				define('WEIXIN_SSLCERT_PATH', APP_PATH . 'Lib/Payment/cert/apiclient_cert.pem');
				define('WEIXIN_SSLKEY_PATH', APP_PATH . 'Lib/Payment/cert/apiclient_key.pem');
				define('WEIXIN_SSLCA_PATH', APP_PATH . 'Lib/Payment/cert/rootca.pem');
			...
			} else if ($detail['order_status'] == 7 && !empty($detail['order_no']) && $detail['pay_name'] == 'weixin') {
				$payment = D('Payment') -> getPayment($detail['pay_name']);
				define('WEIXIN_APPID', $payment['appid']);
				define('WEIXIN_MCHID', $payment['mchid']);
				define('WEIXIN_APPSECRET', $payment['appsecret']);
				define('WEIXIN_KEY', $payment['appkey']);
				define('WEIXIN_SSLCERT_PATH', APP_PATH . 'Lib/cert/appcert/apiclient_cert.pem');
				define('WEIXIN_SSLKEY_PATH', APP_PATH . 'Lib/cert/appcert/apiclient_key.pem');
				define('WEIXIN_SSLCA_PATH', APP_PATH . 'Lib/cert/appcert/rootca.pem');
				include (APP_PATH . 'Lib/Payment/weixin/WxPay.Api.php');
				$pay_price = $detail['pay_price'];
				$input = new WxPayRefund();
				$input -> SetOut_trade_no($detail['order_no']);
			...
			if (is_array($ids)) {
				...
				foreach ($ids as $pay_id) {
					...
					if ($detail['order_status'] == 7 && !empty($detail['order_no']) && $detail['pay_name'] == 'weixin') {
						...
						define('WEIXIN_APPSECRET', $payment['appsecret']);
						define('WEIXIN_KEY', $payment['appkey']);
						define('WEIXIN_SSLCERT_PATH', APP_PATH . 'Lib/cert/cacert/apiclient_cert.pem');
						define('WEIXIN_SSLKEY_PATH', APP_PATH . 'Lib/cert/cacert/apiclient_key.pem');
						define('WEIXIN_SSLCA_PATH', APP_PATH . 'Lib/cert/cacert/rootca.pem');
						include (APP_PATH . 'Lib/Payment/weixin/WxPay.Api.php');
						$pay_price = $detail['pay_price'];
						$input = new WxPayRefund();
						$input -> SetOut_trade_no($detail['order_no']);
					...
					} elseif ($detail['order_status'] == 7 && !empty($detail['order_no']) && $detail['pay_name'] == 'weixin') {
						$payment = D('Payment') -> getPayment($detail['pay_name']);
						define('WEIXIN_APPID', $payment['appid']);
						define('WEIXIN_MCHID', $payment['mchid']);
						define('WEIXIN_APPSECRET', $payment['appsecret']);
