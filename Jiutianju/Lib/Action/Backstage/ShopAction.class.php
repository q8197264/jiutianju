...
class ShopAction extends CommonAction{
    ...
    private $edit_fields = array('user_id', 'cate_id', 'cate_name','grade_id','city_id', 'area_id', 'business_id', 'shop_name', 'mobile', 'logo', 'photo', 'addr', 'tel', 'extension', 'contact', 'tags', 'near', 'business_time', 'delivery_time',  'orderby', 'lng', 'lat', 'price', 'is_ding', 'recognition','panorama_url', 'apiKey', 'mKey', 'partner', 'machine_code', 'service', 'service_audit', 'is_ele_print', 'is_tuan_print', 'is_goods_print', 'is_booking_print','is_appoint_print','service_audit','express_price','commission','end_date');
	
	public function _initialize(){
        parent::_initialize();
        $this->assign('grades',$grades = D('Shopgrade')->where(array('closed'=>0))->select());//二开增加商家等级
    }
    ...
    private function editCheck($shop_id) {
        ...
        if (empty($data['cate_id'])) {
		 ...
		 $data['cate_name'] = (int) $data['cate_name'];
		$data['grade_id'] = (int) $data['grade_id'];
        if (empty($data['grade_id'])) {
            $this->baoError('商家等级不能为空');
        }
