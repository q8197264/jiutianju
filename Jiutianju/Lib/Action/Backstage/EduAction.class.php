   ...
   public function _initialize() {
    ...
    private function createCheck() {
		...
		if($Edu = D('Edu')->where(array('shop_id'=>$data['shop_id']))->find()){
            $this->baoError('您选择的商家【'.$shop['shop_name'].'】已经开通交教育，请不要重复添加');
        }
    ...
    public function delete($edu_id = 0){
        ...
        if(is_numeric($edu_id) && ($edu_id = (int) $edu_id)){
			if($obj->where(array('edu_id' => $edu_id))->save(array('closed' => 1))){
				...
				$Educourse = D('Educourse')->where(array('edu_id' => $edu_id))->select();
				foreach ($Educourse as $k => $value) {
					D('Educourse')->save(array('course_id' => $value['course_id'], 'closed' => 1));
				}
