...
class TuanAction extends CommonAction {
        ...
        if ($code == 'wait') { // 到店付 将不会有支付记录，并且不能使用在线的积分
            ...
            if (D('Tuanorder')->save(array('order_id' => $order_id, 'status' => '-1'))) { //更新成到店付的状态
			   ...
			   if($this->_CONFIG['sms']['dxapi'] == 'dy'){
                ...
                $shopsms = D('Sms_shop')->find($tuan['shop_id']);
                $contnum=$shopsms['num'];
				if ($contnum > 0){
		...
		//优惠劵结束
			...
			if(!empty($config['integral']['buy'])){
				...
				$integral_price = $used;
                    ...
                    $order['need_pay'] = $order['total_price'] - $integral_price-$coupon_price;//这里有修改
    ...
    public function pay() {
		...
		$this->assign('coupon_download_list', D('Coupondownload')->where(array('user_id'=>$uid,'is_used'=>0))->order(array('create_time' => 'desc'))->select());
