...
class ShopAction extends CommonAction{
    ...
    public function apply(){
        if (empty($this->uid)) {
		 ...
		 $kg=$this->_CONFIG['shop']['shop_baozhengjing'];  
		if($kg == 1){
$bzjmoney=$this->_CONFIG['shop']['shop_pay_bzj'] * 100;
		if ($this->member['money'] < $bzjmoney) {
			$info=$bzjmoney/100;
            $this->error('余额不足'.$info.'元，请充值后再申请开通店铺！', U('Members/money/money'));
        }
		}
		...
		if($kg == 1){
				...
				D('Users')->addMoney($this->uid, -$bzjmoney, '商城入驻保证金扣费'.$info.'元');
				D('Users')->save(array('user_id'=>$this->uid,'frozen_gold'=>$bzjmoney));
				D('Usergoldlogs')->add(array(
			   'user_id' => $this->uid,
			   'gold'=>$bzjmoney,
			   'intro' => '商户入驻全民消保计划冻结'.$info.'元保证金',
			   'create_time' => NOW_TIME,
			   'create_ip'  => get_client_ip()
		   ));

				}
                $this->baoSuccess('恭喜您申请成功,系统已扣款'.$info.'元，请等待管理员审核！', U('shop/index'));
            }
            $this->baoError('申请失败！');
        } else {
            $areas = D('Area')->fetchAll();
    ...
    private function createCheck(){
        $data = $this->checkFields($this->_post('data', false), array('cate_id','user_guide_id', 'tel', 'mobile','qq', 'logo', 'photo', 'shop_name', 'contact', 'details', 'business_time', 'city_id', 'area_id', 'business_id', 'addr', 'lng', 'lat', 'recognition','is_pei','yx','end_date'));
		$guide_ids = htmlspecialchars($data['user_guide_id']);
		$cates = D('Shopcate')->where(array('cate_id' => $data['cate_id']))->find();
		...
		if(!empty($data['qq'])){
			$this->baoError('商家QQ不能为空',2000,true);
	}
		...
		$dj=$this->_CONFIG['shop']['shop_dj'];
		$data['grade_id']= $dj;
		$data['end_date']= $end_date;
		$au=$this->_CONFIG['shop']['shop_pe'];
