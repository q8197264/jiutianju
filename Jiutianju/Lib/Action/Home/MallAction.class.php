    ...
    public function index(){
        ...
        switch ($order) {
            ...
            case 'v':
                ...
                break;
            default:
                $orderby = array('is_vs5' => 'desc', 'orderby' => 'asc', 'sold_num' => 'desc', 'goods_id' => 'desc');
                $linkArr['order'] = $order;
                break;
        }
        ...
        $this->assign('cate', D('Goods_cate')->where(array('parent_id' => 0))->select());
   ...
   public function get_spec($goods_id){
	 ...
	 foreach($goods_ids as $v){
        ...
        foreach ($goods as $val) {
			...
			$num[$val['goods_id']] = $num[$val['goods_spec']];
				...
				$need_pay = $dan_price - $use_price - $coupon_price;//实际支付金额
				$js_price = $val['settlement_price'] * $num[$val['goods_id']];
				$tprice += $price;
        ...
        //总订单
        $order = array('user_id' => $this->uid, 'create_time' => NOW_TIME, 'create_ip' => $ip, 'need_pay' => $need_pay, 'goods_id' => $val['goods_id'], 'total_price' => 0,'is_fanli'=>$val['is_fanli']);
    ...
    public function paycode2(){
        ...
        foreach ($orders as $k => $val) {
            $need_pay[$val[order_id]] = $val['total_price'] -$coupon['reduce_price'];
            D('Order')->where(array('order_id' => $val['order_id']))->save(array('need_pay' => $need_pay[$val[order_id]]));
        }
        /*****************************************************/
		 $payment = $this->_param('payment', 'htmlspecialchars');
        $code = '';
        switch ($payment) {
    ...
    public function pay2(){
        ...
        if ($code == 'wait') {
			...
			D('Order')->save(array('order_id' =>$order_id,'download_id' =>$download_id,'coupon_price' =>$coupon_price));//重新再写一次优惠券
			D('Ordergoods')->save(array('order_id' =>$order_id,'download_id' =>$download_id,'coupon_price' =>$coupon_price,'need_pay' => $need_pay));//重新再写一次优惠券
            D('Weixintmpl')->weixin_notice_goods_user($order_id,$this->uid,1);//商城微信通知
			D('Coupondownload')->save(array('download_id' => $download_id, 'is_used' => 1,'used_time' => NOW_TIME,'used_ip' => get_client_ip()));
