...
class SmsModel extends CommonModel{
    ...
    public function sms_yzm($mobile, $randstring){
        $config = D('Setting')->fetchAll();
        if ($config['sms']['dxapi'] == 'dy') {
            D('Sms')->DySms($config['site']['sitename'], 'sms_yzm',$shop_id = '0', $mobile, array(
			'sitename' => $config['site']['sitename'], 
			'code' => $randstring));
        } else {
    ...
    public function sms_user_newpwd($mobile, $password){
        $config = D('Setting')->fetchAll();
        if ($config['sms']['dxapi'] == 'dy') {
            D('Sms')->DySms($config['site']['sitename'], 'sms_user_newpwd',$shop_id = '0', $mobile, array(
				'sitename' => $config['site']['sitename'], 
				'newpwd' => $password
			));
        } else {
    ...
    public function sms_tuan_user($uid, $order_id){
        ...
        //发送团购劵
        if ($config['sms']['dxapi'] == 'dy') {
            D('Sms')->DySms($config['site']['sitename'], 'sms_tuan_user',1, $user['mobile'], array(
				'code' => $codestr, 
				'user' => $user['nickname'], 
				'shop_name' => $tuan_title
			));
        } else {
            D('Sms')->sendSms('sms_tuan_user', 1,$user['mobile'], array(
				'code' => $codestr, 
				'user' => $user['nickname'], 
				'shop_name' => $tuan_title
			));
  ...
  public function tuanTZshop($shop_id){
		...
		$shopsms = D('Sms_shop')->find($shop_id);
        $config = D('Setting')->fetchAll();
        
        if ($config['sms']['dxapi'] == 'dy') {
            $this->DySms($config['site']['sitename'], 'sms_tuan_tz_shop',$shop['mobile'], array('sitename' => $config['site']['sitename']));
        } else {
            $this->sendSms('sms_tuan_tz_shop',$shop['shop_id'], $shop['mobile'], array());
        }
        $newnum=$shopsms['num']-1;
		D('Sms_shop')->save(array('shop_id'=>$shop['shop_id'],'num'=>$newnum));
        return true;
