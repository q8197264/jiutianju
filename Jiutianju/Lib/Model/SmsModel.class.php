...
class SmsModel extends CommonModel{
    ...
    public function sendSms($code, $mobile, $data){
        ...
        if (!empty($tmpl[$code]['is_open'])) {
            ...
            $local = array('mobile' => $mobile, 'content' => urlencode($content));
    ...
    //jack add
    public function eleTZshopPSY($order_id){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            $config = D('Setting')->fetchAll();
            //$shop = D('Shop')->find($order['shop_id']);
            //获取所有激活的配送员
            $delivery_list = D("Delivery")->field("mobile")->where("d_status=1 and is_fa=1")->select();
            if(!empty($delivery_list)){
                ...
                foreach($delivery_list as $key => $val){
                    $mobile = $val['mobile'];
                    if ($config['sms']['dxapi'] == 'dy') {
                        $this->DySms($config['site']['sitename'], 'sms_delivery_tz_delivery_id',$mobile, array(
                            'sitename' => $config['site']['sitename'], 
                            'sitename' => $config['site']['sitename']
                        ));
                    } else {
                        $code = 'sms_delivery_tz_delivery_id';
                        if (!empty($tmpl[$code]['is_open'])) {
                            $content = $tmpl[$code]['sms_tmpl'];
                            $data['sitename'] = $config['site']['sitename'];
                            $data['tel'] = $config['site']['tel'];
                        ...
                        //$this->sendSmsnoretu('sms_delivery_tz_delivery_id', $mobile, array());
                    }
                }
            }
        }
    ...
    public function sendSmsnoretu($code, $mobile, $data){
